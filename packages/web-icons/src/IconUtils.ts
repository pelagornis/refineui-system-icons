...
class WebIconUtils {
  ...
  // === FLUENTUI 스타일 방식 ===
  // 동적 아이콘 생성 메서드
  createIconMethod(iconName: string, props: IconProps = {}): string | null {
  ...
  private createUnsizedIcon(iconName: string, style: 'regular' | 'filled', props: IconProps = {}): string | null {
  ...
  // Sized 아이콘 생성
  private createSizedIcon(iconName: string, size: number, style: 'regular' | 'filled', props: IconProps = {}): string | null {
    const iconData = this.metadata.icons[iconName];
    if (!iconData) return null;
...
// 싱글톤 인스턴스 생성
const webIconUtils = new WebIconUtils();

export default webIconUtils;
export { WebIconUtils };
